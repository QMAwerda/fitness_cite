package com.mirea.fitness.controllers;
...
import com.mirea.fitness.services.*;
    ...
    private String createMessageForUser(List<Basket> userBaskets) {
        ...
        for (int i = 0; i < userProducts.size(); i++) {
            res += (i + 1) + ") " + userProducts.get(i).getName() + " (Количество: "
                    + userBaskets.get(i).getProductCount() + ", Стоимость: "
                    + (userProducts.get(i).getPrice() * userBaskets.get(i).getProductCount()) + " р.)<br>";
        }
